...
// directly. In this mode, only the default templates are supported (for other
// templates, an error is printed to ErrWriter), and the only custom functions
// used are "wrap" and "wrapAt".
func DefaultPrintHelpCustom(out io.Writer, templ string, data any, customFuncs map[string]any) {
	const maxLineLength = 10000

	cmd, ok := data.(*Command)
	if !ok {
		reportNoTemplate(nil)
		return
	}
...
// so it's not silently ignored.
func reportNoTemplate(cmd *Command) {
	w := ErrWriter
	if cmd != nil && cmd.Root().ErrWriter != nil {
		w = cmd.Root().ErrWriter
	}
	_, _ = fmt.Fprintln(w, errNoTemplate)
}
